...
def main(config_file):
    ...
    epochs_per_stage = c_train.get('epochs_per_stage', c_train['epochs']) # Epochs for each depth stage
    start_iteration = c_model.get('start_iteration', 1) # Allow starting from a specific depth
    lr_decay_factor = c_train.get('lr_decay_factor', 0.7) # Decay factor for LR between stages - Tuned default
    # --- End Train-and-Unroll config ---
    ...
    model = MgNO(lifting = None,
                proj = None,
                dim_input = 4,
                features=c_model.get('features', 24), # Use config value
                loss = c_model['loss'],
                learning_rate = c_train['lr'], # Initial LR from config
                step_size= c_train['step_size'], # Still needed if using StepLR? OneCycleLR uses epochs.
                gamma= c_train['gamma'],
                weight_decay= c_train['weight_decay'],
                epochs=epochs_per_stage, # Use epochs per stage for scheduler
                eta_min= c_train['eta_min'],
                iteration = 1 # Start with iteration 1, will be updated in loop
                )
    print("Initialized new MgNO model from scratch.")

    # DataLoaders (assuming they don't change between stages)
    if c_proj['do'] == 'train':
        train_dataloader, val_dataloader = datasetFactory(config=config, do=c_proj['do'])
    elif c_proj['do'] == 'test':
        # Test mode doesn't use the Train-and-Unroll loop
        val_dataloader = datasetFactory(config=config, do=c_proj['do'])
        # Load the final desired model for testing
        ...
        trainer = pl.Trainer(accelerator=c_proj['accelerator'],
                             devices=1 if c_proj['devices'] == 1 else [i for i in range(c_proj['devices'])],
                             logger=CSVLogger("logs", name="test_run"),
                             )
        trainer.validate(model, val_dataloader)
        return # End execution for test mode

    # --- Train-and-Unroll Loop ---
    last_ckpt_path = None # Start without resuming from a checkpoint
    current_lr = c_train['lr'] # Initialize LR for the first stage

    for current_depth in range(start_iteration, max_iterations + 1):
        print(f"\n--- Starting Training Stage: Depth {current_depth}/{max_iterations} ---")
        print(f"Setting learning rate for this stage: {current_lr}")
        model.iteration = current_depth
        model.epochs = epochs_per_stage # Ensure scheduler uses correct epochs for this stage
        model.learning_rate = current_lr # Set the LR for the current stage

        # Configure Checkpoint Callback for this stage
        stage_save_dir = os.path.join(config["ckpt"]["PATH"], config["ckpt"]["save_dir"], f"depth_{current_depth}")
        checkpoint_callback = ModelCheckpoint(
                                dirpath=stage_save_dir,
                                every_n_epochs = 1,
                                save_last = True,
                                monitor = 'val_loss',
                                mode = 'min',
                                save_top_k = c_proj['save_top_k'],
                                filename=f"model-depth{current_depth}"+"-{epoch:03d}-{val_loss:.4f}-{loss:.4f}", # Use 'loss' not 'loss_epoch' if logged as 'loss'
                            )

        # Configure Trainer for this stage
        lr_monitor = LearningRateMonitor(logging_interval='step')
        csv_logger = CSVLogger("logs", name=f"depth_{current_depth}_logs") # Separate logs per depth

        trainer = pl.Trainer(max_epochs=epochs_per_stage, # Train for fixed epochs per stage
                            accelerator=c_proj['accelerator'],
                            devices = c_proj['devices'] if isinstance(c_proj['devices'], list) else ([i for i in range(c_proj['devices'])] if c_proj['devices'] > 1 else 1),
                            callbacks = [checkpoint_callback, lr_monitor],
                            logger=csv_logger,
                            strategy='ddp_find_unused_parameters_true' if c_proj['devices'] > 1 else 'auto', # Simplified strategy selection
                            # precision='16-mixed' # Optional: Enable mixed precision
                            )

        # Fit the model for the current stage
        # Load 'last' checkpoint from the *previous* stage if current_depth > start_iteration
        print(f"Fitting model for depth {current_depth}...")
        trainer.fit(model, train_dataloader, val_dataloader, ckpt_path='last' if current_depth > start_iteration else None) # Use 'last' to resume from previous stage checkpoint, None for first stage

        # Update last_ckpt_path for the next stage to resume from the best/last of this stage
        last_ckpt_path = trainer.checkpoint_callback.last_model_path # Use last checkpoint to continue sequence
        print(f"--- Finished Training Stage: Depth {current_depth} ---")
        print(f"Checkpoint saved for this stage: {last_ckpt_path}")

        # Decay learning rate for the next stage
        current_lr *= lr_decay_factor
        print(f"Decaying learning rate for next stage to: {current_lr}")

        # Optional: Add validation run after each stage if needed
        # print(f"Validating model at depth {current_depth}...")
        # trainer.validate(model, val_dataloader, ckpt_path=last_ckpt_path)


    # --- End Train-and-Unroll Loop ---

    print("\nTrain-and-Unroll process finished.")
    if c_proj['save'] == True and last_ckpt_path:
        final_save_path = os.path.join(c_proj['save_path'], c_proj['save_name'])
        # Save the final model state (optional, as checkpoints exist)
        # Option 1: Save the whole model object (less recommended for PL)
        # torch.save(model, final_save_path)
        # Option 2: Copy the last checkpoint
        try:
            shutil.copyfile(last_ckpt_path, final_save_path)
            print(f'Final model checkpoint copied to: {final_save_path}')
        except Exception as e:
            print(f"Could not copy final checkpoint: {e}")
            print(f"Final checkpoint remains at: {last_ckpt_path}")
