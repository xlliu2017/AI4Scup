...
class Fourier_layer(nn.Module):
    def __init__(self,  features_, wavenumber, activation = 'gelu', is_last = False):
...
def main(config_file):
    ...
    train_dataloader, val_dataloader, normalizer = datasetFactory(config=config, do=c_proj['do'])
    ...
    if c_proj['devices'] == 1 :
        ...
        trainer = pl.Trainer(max_epochs=max_epochs,
                            ...
                            devices = 'auto',
                            callbacks = [checkpoint_callback,lr_monitor],
                            strategy = 'ddp',gradient_clip_val=0.8)
    trainer.fit(model, train_dataloader, val_dataloader)
    if c_proj['save'] == True:
        save_path = os.path.join(c_proj['save_path'], 'submission.pt')
