...
class MgNO(pl.LightningModule):
    ...
    def configure_optimizers(self, optimizer=None, scheduler=None):
        ...
        if  scheduler is None:
            #scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=self.step_size, gamma=self.gamma)
            # scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max = self.step_size, eta_min= self.eta_min)
            scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=1e-5, steps_per_epoch=1, epochs=5, pct_start=0.1, anneal_strategy='cos', div_factor=1.0, final_div_factor=10.0,)
        return {
        "optimizer": optimizer,
        "lr_scheduler": {
            "scheduler": scheduler
...
def main(config_file):
    ...
    #             eta_min= c_train['eta_min'],
    #             )
    # model.load_state_dict(torch.load('/ibex/user/liux0t/AI4S-cupv2/save_files/MgNOv6.1/model-epoch=003-val_loss=0.0046.ckpt')['state_dict']) #'/ibex/user/liux0t/AI4S-cupv2/save_files/FNO/model-epoch=010-val_loss=0.0311.ckpt'

    model = torch.load('/ibex/user/liux0t/AI4S-cupv2/submission16.1.pt')
    model.iteration = 5
    print(model)
    model.criterion = LpLoss()
    model.criterion_val = nn.MSELoss()
    ...
    if c_proj['do'] == 'train':
        ...
        if c_proj['save'] == True:
            save_path = os.path.join(c_proj['save_path'], 'submission14.2.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c','--config_file', type=str, 
                                ...
                                default='/ibex/user/liux0t/AI4S-cupv2/config/FNOtest.yaml')
